...
class CarSerializer(serializers.ModelSerializer):
    ...
    def validate_color(self, v):
        if not v.startswith("#") or len(v) not in (4, 7):
            raise serializers.ValidationError("Must be a valid hex color e.g. #FF0000 or #FFF")
        return v.upper()
...
class CartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Cart
        fields = ["cart_id", "branch_id", "branch_name", "restaurant_name", "items", "total"]
...
class ConfirmOrderSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if method == Payment.Method.STRIPE and not attrs.get("stripe_intent_id"):
            raise serializers.ValidationError({"stripe_intent_id": "Required for Stripe payment."})
...
class PaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model  = Payment
        fields = ["id", "method", "status", "amount", "stripe_intent_id",
                  "cash_received_at", "created_at"]
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = OrderItem
        fields = ["id", "name", "price", "options_price", "quantity",
...
class OrderETASerializer(serializers.Serializer):
    ...
    arrived_lat           = serializers.DecimalField(max_digits=9, decimal_places=6, allow_null=True)
    arrived_lon           = serializers.DecimalField(max_digits=9, decimal_places=6, allow_null=True)