...
from .serializers import (
    ...
    CashReceiveSerializer,
    QRScanSerializer,
    # user actions
    MarkArrivedSerializer,
    FeedbackSerializer,
)
...
def _validate_options(selected_option_ids: list, item: MenuItem):
    ...
    for g in groups:
        ...
        if g.type == "required" and count < g.min_select:
            raise ValueError(
                f"Group '{g.name}' requires at least {g.min_select} selection(s)."
            )
        if count > g.max_select:
            raise ValueError(
                f"Group '{g.name}' allows at most {g.max_select} selection(s)."
            )
...
def _order_or_404(order_id, extra_filter=None):
    ...
    qs = Order.objects.select_related(
        "branch", "branch__restaurant", "car", "payment", "customer"
...
class CartView(APIView):
    ...
    def post(self, request):
        ...
        if not item:
            return APIResponse.error(
                message="Menu item not found or unavailable.", status_code=404
            )
        ...
        try:
            ...
            return APIResponse.error(
                errors={"selected_options": [str(exc)]}, message=str(exc)
            )

        # Prevent multi-branch carts
        other_cart = (
            Cart.objects.filter(customer=request.user).exclude(branch=branch).first()
        )
        if other_cart:
            return APIResponse.error(
    ...
    def delete(self, request):
        ...
        if not cart:
            return APIResponse.error(
                message="No active cart for this branch.", status_code=404
            )
        cart.delete()
...
class CartItemView(APIView):
    ...
    def patch(self, request, pk):
        ...
        return APIResponse.success(
            message="Cart item updated.", data=CartSerializer(ci.cart).data
        )
    ...
    def delete(self, request, pk):
        ...
        return APIResponse.success(
            message="Item removed from cart.", data=CartSerializer(cart).data
        )
        ...
        if d["payment_method"] == Payment.Method.STRIPE:
            try:
                ...
                intent = stripe.PaymentIntent.create(
                    amount=int(
                        total * 100
                    ),  # Stripe uses smallest currency unit (halalas)
                    currency="sar",
                    metadata={
...
class OrderListView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == "customer":
            ...
            if not emp:
                return APIResponse.error(
                    message="Employee profile not found.", status_code=404
                )
...
class AcceptOrderView(APIView):
    ...
    def post(self, request, order_id):
        ...
        if order.status != Order.Status.ORDER_SENT:
            return APIResponse.error(
                message="Only ORDER_SENT orders can be accepted.", status_code=400
            )
...
class MarkArrivedView(APIView):
    ...
    def post(self, request, order_id):
        ...
        mark_user_arrived.delay(
            ...
            latitude=(
                float(s.validated_data["latitude"])
                if s.validated_data.get("latitude")
                else None
            ),
            longitude=(
                float(s.validated_data["longitude"])
                if s.validated_data.get("longitude")
                else None
            ),
        )

        return APIResponse.success(
            message="Arrival noted. The restaurant has been informed."
        )
...
class OrderETAView(APIView):
    ...
    def get(self, request, order_id):
        ...
        distance = _haversine_km(
            order.arrived_lat, order.arrived_lon, branch_lat, branch_lon
        )
    ...
    def post(self, request):
        ...
        order = (
            Order.objects.select_related(
                "branch", "branch__restaurant", "car", "payment"
            )
        ...
        if not order:
            return APIResponse.error(
                message="Invalid QR code or order not ready.", status_code=404
            )
        if not _can_manage(request.user, order):
            return APIResponse.error(message="Unauthorized.", status_code=403)
...
class DeliverManualView(APIView):
    ...
    def post(self, request, order_id):
        ...
        if not order:
            return APIResponse.error(
                message="Order not found or not ready.", status_code=404
            )
...
class FeedbackView(APIView):
    ...
    def get(self, request, order_id):
        ...
        if not feedback:
            return APIResponse.error(
                message="No feedback submitted yet.", status_code=404
            )
    ...
    def post(self, request, order_id):
        ...
        if not order:
            return APIResponse.error(
                message="Order not found or not delivered yet.", status_code=404
            )
        if hasattr(order, "feedback"):
            return APIResponse.error(
                message="Feedback already submitted.", status_code=400
            )
