    ...
    def validate(self, attrs):
        ...
        if min_s > max_s:
            raise serializers.ValidationError(
                {"min_select": "min_select cannot exceed max_select."}
            )
        return attrs
...
class MenuItemWriteSerializer(serializers.ModelSerializer):
    ...
    def validate_category_id(self, value):
        ...
        if (
            branch
            and not MenuCategory.objects.filter(id=value.id, branch=branch).exists()
        ):
            raise serializers.ValidationError(
                "Category does not belong to this branch."
            )
        return value
...
class ModifierOptionWriteSerializer(serializers.Serializer):
    ...
    option_type = serializers.ChoiceField(
        choices=ModifierOption.OptionType.choices,
        default=ModifierOption.OptionType.FREE,
    )
...
class ModifierGroupBulkSerializer(serializers.Serializer):
    ...
    type = serializers.ChoiceField(
        choices=ModifierGroup.Type.choices, default=ModifierGroup.Type.OPTIONAL
    )
